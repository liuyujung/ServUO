	...
	public class YardTarget : Target
	{
		...
		public void EndPlace( Point3D loc )
		{
			...
			if ( m_From.Backpack.ConsumeTotal( typeof( Gold ), m_Price )
			    //daat start
			    || (OWLTROptionsManager.IsEnabled(OWLTROptionsManager.OPTIONS_ENUM.USE_STORAGE_RESOURCES) && MasterStorageUtils.ConsumeCurrency(m_From as PlayerMobile, typeof(Gold), m_Price))
			    //daat end
			    || m_From.BankBox.ConsumeTotal( typeof(Gold), m_Price ) )
			{
				switch (m_ItemID)
				{
