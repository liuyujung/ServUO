...
namespace Xanthos.ShrinkSystem
{
	public class ShrinkCommands
	{
		...
		[Description( "Disables all ShrinkItemXs in the world." )]
		...
		static private void SetLockDown( Mobile from, bool lockDown )
		{
			...
			{
				World.Broadcast( 0x35, true, "A server wide ShrinkItemX lockout has initiated." );
				World.Broadcast( 0x35, true, "All shrunken pets have will remain shruken until further notice." );
			}
			else
			{
				World.Broadcast( 0x35, true, "The server wide ShrinkItemX lockout has been lifted." );
	...
	public class ShrinkTarget : Target
	{
		...
		protected override void OnTarget( Mobile from, object target )
		{
			...
			else if ( !m_StaffCommand && ShrinkItemX.IsPackAnimal( pet ) && ( null != pet.Backpack && pet.Backpack.Items.Count > 0 ) )
			...
			else
			{
				...
				from.PlaySound( 492 );
				from.AddToBackpack( new ShrinkItemX( pet ) );
