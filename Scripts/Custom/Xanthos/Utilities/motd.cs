		...
		internal class AccountGump : Gump
		{
			public AccountGump( Mobile from, NetState state, bool showOnlyCredits ) : base( 30, 20 )
			{
				...
				if ( !showOnlyCredits )
				{
					...
					AddLabel( 205, 180, NewsGump.kDataColor, (( Convert.ToInt64(state.Flags) & 0x10 ) != 0 ) ? "Samurai Empire" : (( Convert.ToInt64(state.Flags) & 0x08 ) != 0) ? "Age of Shadows" : (( Convert.ToInt64(state.Flags) & 0x04 ) != 0) ? "Blackthorn's Revenge" : (( Convert.ToInt64(state.Flags) & 0x02 ) != 0 ) ? "Third Dawn" : (( Convert.ToInt64(state.Flags) & 0x01 ) != 0 ) ? "Renaissance" : "The Second Age" );
